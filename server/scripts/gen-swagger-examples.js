...
const SWAGGER_PATH = path.join(__dirname, "../docs/swagger.yaml");
...
const commentContent = doc?.paths?.["/comments/{offerId}"]?.post?.requestBody?.content?.["application/json"];
if (commentContent) {
...
if (offerContent) {
  offerContent.examples = {
    generated: {
      summary: "Generated example (text fields only)",
